...
data "azurerm_virtual_network" "dns" {
  ...
  resource_group_name = var.vnet_links[count.index].rg_name
  name                = var.vnet_links[count.index].virtual_network_name
}
...
resource "azurerm_private_dns_zone_virtual_network_link" "vnet-link" {
  ...
  resource_group_name   = data.azurerm_virtual_network.dns[count.index].resource_group_name
  virtual_network_id    = data.azurerm_virtual_network.dns[count.index].id
